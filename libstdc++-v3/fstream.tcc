...
namespace std _GLIBCXX_VISIBILITY(default)
{
		...
		_M_allocate_internal_buffer()
	{
		...
		{
			/**
			 * Use new keyword to allocate buffer for internal usage.
			 */
		...
		open(const char *__s, ios_base::openmode __mode)
	{
		...
		// Make sure we do not re-open same file.
		if (!this->is_open())
		{
			...
			if (this->is_open())
			{
				...
				// 27.8.1.3,4
				if ((__mode & ios_base::ate) && this->seekoff(0, ios_base::end, __mode) == pos_type(off_type(-1)))
					this->close();
				else
					__ret = this;
			}
	...
	basic_filebuf<_CharT, _Traits>::
		close()
	{
		// if file is closed already, return directly.
		if (!this->is_open())
			return 0;
		...
		showmanyc()
	{
		...
		if (__testin && this->is_open())
		{
			...
			if (__check_facet(_M_codecvt).encoding() >= 0 && __testbinary)
#else
		...
		seekoff(off_type __off, ios_base::seekdir __way, ios_base::openmode)
	{
		...
		if (this->is_open() && !__testfail)
		{
			...
			bool __no_movement = __way == ios_base::cur && __off == 0 && (!_M_writing || _M_codecvt->always_noconv());
				...
				if (_M_reading)
				{
					if (__check_facet(_M_codecvt).always_noconv())
					{
						if (_M_codecvt_tmp && !__check_facet(_M_codecvt_tmp).always_noconv())
							__testvalid = this->seekoff(0, ios_base::cur, _M_mode) != pos_type(off_type(-1));
					}
					else
					{
						// External position corresponding to gptr().
						_M_ext_next = _M_ext_buf + _M_codecvt->length(_M_state_last, _M_ext_buf,
																	  _M_ext_next,
																	  this->gptr() - this->eback());
						const streamsize __remainder = _M_ext_end - _M_ext_next;
						if (__remainder)
