...
struct _Rb_tree_node_base
{
  ...
  // return pointer to the leftmost/minimum node
  static _Base_ptr _S_minimum(_Base_ptr __x)
  {
    while (__x->_M_left != 0) __x = __x->_M_left;
    return __x;
  }
  // return pointer to the rightmost/maximum node
...
{
  // _link_type is a pointer type to node struct
...
_Rb_tree_rotate_left(_Rb_tree_node_base* __x, _Rb_tree_node_base*& __root)
{
  ...
  /**
   * @brief 
   *    Let __y's left child be the right child of x.
   */
  __x->_M_right = __y->_M_left;             // Let __y's left child be the right child of x
  if (__y->_M_left !=0)                     // If __y's left child isn't NILL
    __y->_M_left->_M_parent = __x;          // Let __y's left child's parent be x
  
  /**
   * @brief
   *    Let __y be the child of __x's parent.
   */ 
  __y->_M_parent = __x->_M_parent;          // Let __x's parent be __y's parent
  if (__x == __root)
    __root = __y;
  else if (__x == __x->_M_parent->_M_left)  // If new node is the left child of its parent
    __x->_M_parent->_M_left = __y;          // Let __y become the left child of x's parent
  else                                      // If target node is the right child
    __x->_M_parent->_M_right = __y;         // Let __y become the right child of x's parent

  /**
   * @brief
   *    Let __x become the left child of __y.
   */
  __y->_M_left = __x;                       // __x becomes __y's left child
  __x->_M_parent = __y;                     // __x's parent becomes __y

  //
  // Note that: __y->right > __y > __y->left > __x
  // We finally change the structure to the diagram below.
  //
  //                  __y
  //            /             \
  //         __x           __y->right
  //       /     \             /    \
  // subtree   __y->left   subtree  subtree
  //
}
...
_Rb_tree_rebalance(_Rb_tree_node_base* __x, _Rb_tree_node_base*& __root)
{
  ...
  {
    // If x's parent is the left child of x's grandparent 
    if (__x->_M_parent == __x->_M_parent->_M_parent->_M_left) 
    { 
      // __y is the uncle of __X.
      _Rb_tree_node_base* __y = __x->_M_parent->_M_parent->_M_right;
      // If __x's uncle's color is red.
      // In this case, we just need to change colors.
      if (__y && __y->_M_color == _S_rb_tree_red) 
      {
        __x->_M_parent->_M_color = _S_rb_tree_black;
        __y->_M_color = _S_rb_tree_black;
        __x->_M_parent->_M_parent->_M_color = _S_rb_tree_red;
        __x = __x->_M_parent->_M_parent;  // go up to continue
      }
      else  // If __x's right uncle's color is black
      {  
		    // If new node is the right child of its parent
        if (__x == __x->_M_parent->_M_right) 
        {
          __x = __x->_M_parent;
          _Rb_tree_rotate_left(__x, __root); // new node becomes the parent
        }
        __x->_M_parent->_M_color = _S_rb_tree_black;
        __x->_M_parent->_M_parent->_M_color = _S_rb_tree_red;
    ...
    else  // If x's parent is the right child of x's grandparent 
    {
                             ...
                             _Rb_tree_node_base*& __rightmost)
{
  ...
  if (__y->_M_left == 0)     // __z has at most one non-null child. y == z.
    __x = __y->_M_right;     // __x is right child of __y and it might be null.
  else
    if (__y->_M_right == 0)  // __z has exactly one non-null child. y == z.
      __x = __y->_M_left;    // __x is the left child of y and it must be non-null.
    else {                   // __z has two non-null children.  
      __y = __y->_M_right;   // Set __y to __z's successor.  __x might be null.
      while (__y->_M_left != 0)
        __y = __y->_M_left;
      __x = __y->_M_right;
    }
  ...
  if (__y != __z) {   // relink y in place of z.  y is z's successor
    ...
    else  // If __z is the left child of its parent
     ...
     *    rightmost node means it doesn't have right child.
     */
    if (__leftmost == __z)            // This implies __z doesn't have left child
      if (__z->_M_right == 0)         // __z doesn't have right child
        __leftmost = __z->_M_parent;  // __z's children are null, no children
                                      // makes __leftmost == _M_header if __z == __root
      else                            // __z->right != nullptr
        // __x replace __y, so we find the smallest/leftmost child of __x.
        __leftmost = _Rb_tree_node_base::_S_minimum(__x);

    if (__rightmost == __z)           // This implies __z doesn't have right child
      if (__z->_M_left == 0)          // __z->_M_right must be null also
        __rightmost = __z->_M_parent; // __z's doesn't have children.
                                      // makes __rightmost == _M_header if __z == __root
      else                            // __z->left != nullptr
        // __x replace __y, so we find the biggest/rightmost child of __x.
        __rightmost = _Rb_tree_node_base::_S_maximum(__x);
  }

  /**
   * @brief 
   *    Rebalance the tree only if __y(deleted node) is black or NILL(also black)
   */
  if (__y->_M_color != _S_rb_tree_red) { 
    ...
    // If __x is root or __x is red, change it to black.
    // because __x replace the original position of __y, if __y is black, then one black node disappeared.
    // We need turn __x to black to restore that one black node.
  }
  return __y;
}
